...
// MARK: -  自定义相册类型枚举
struct HDPhotoAlbumType: OptionSet {
    
    let rawValue: Int
    
    /// 用户智能相册 PHAssetCollectionType: smartAlbum PHAssetCollectionSubtype: smartAlbumUserLibrary
    static let smartAlbum = HDPhotoAlbumType(rawValue: 1 << 0)
    
    /// 用户创建的相册 fetchTopLevelUserCollections
    static let userAlbum = HDPhotoAlbumType(rawValue: 1 << 1)
    
    /// iPhone中同步的相册 PHAssetCollectionType: album PHAssetCollectionSubtype: albumSyncedAlbum
    static let syncedAlbum = HDPhotoAlbumType(rawValue: 1 << 2)
    
    /// iCloud中同步的相册 PHAssetCollectionType: album PHAssetCollectionSubtype: albumMyPhotoStream
    static let iCloudAlbum = HDPhotoAlbumType(rawValue: 1 << 3)
    
    /// iCloud分享的相册 PHAssetCollectionType: album PHAssetCollectionSubtype: albumCloudShared
    static let iCloudShared = HDPhotoAlbumType(rawValue: 1 << 4)
    
    /// 所有相册
    static let all: HDPhotoAlbumType = [.smartAlbum, .userAlbum, .syncedAlbum, .iCloudAlbum, .iCloudShared]
    
}

// MARK: -  系统相册管理器
class HDPhotoImageManager {
    ...
    /// - Returns: 相册集合
    static func getPhotoAlbums(with albumType: HDPhotoAlbumType = [.smartAlbum, .userAlbum], filterEmpty: Bool = true) -> [PHAssetCollection] {
