...
class HDImageModel: NSObject {
    ...
    /// 加载缩略图
    func requestThumbImage(_ completion: ((_ model: HDImageModel, _ thumbImage: UIImage?) -> ())?) {
        if let thumbImage = thumbImage {
            completion?(self, thumbImage)
        } else {
            HDPhotoImageManager.requestThumbImage(for: phAsset) { [weak self] (image, finished) in
    ...
    static func == (lhs: HDImageModel, rhs: HDImageModel) -> Bool {
