...
class HDAlbumModel: NSObject {
    ...
    init(_ assetCollection: PHAssetCollection, mediaTypes: [PHAssetMediaType] = [.image, .video]) {
        ...
        fetchAssets = HDPhotoImageManager.getPhotoAssets(for: assetCollection, with: mediaTypes)
    }
    
    /// 获取封面图
    func requestCoverImage(_ completion: ((_ model: HDAlbumModel, _ coverImage: UIImage?) -> ())?) {
        if let coverImage = coverImage {
            ...
            HDPhotoImageManager.requestThumbImage(for: firstAsset) { [weak self] (image, finished) in
    ...
    static func == (lhs: HDAlbumModel, rhs: HDAlbumModel) -> Bool {
